...
import { SHEET_CATEGORIES, UP_TO_SHEET } from './config.js';
...
function parseUpCSV(text) {
  ...
  for (const row of result.data) {
    ...
    txs.push({
      ...
      payee: row[headers[iPayee]] || row[headers[iDesc]] || "",
      amount: Math.abs(amt),
      upCategory: upCat,
      category: sheetCat,
      source: "Up",
      needsReview: !sheetCat,
    });
  }
  return txs;
}
...
Rules: skip PENDING, skip credits, amount=positive, omit mortgage repayments`,
    ...
    return JSON.parse(clean).map(t => ({ ...t, payee: t.payee.replace(/^PENDING TRANSACTION - Purchase - /, ''), source: "AMP", needsReview: false }));
  }
